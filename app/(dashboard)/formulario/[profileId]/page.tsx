  ...
  return (
    <>
      ...
      <div className="w-full h-full p-6 flex flex-col sm:px-16 sm:py-12 sm:min-h-[calc(100vh-176px)] lg:container lg:mx-auto lg:min-h-[calc(100vh-192px)]">
        ...
        {formStep === "0" && <PersonalDataForm currentForm={data.form} profileId={profileId} isEditing={isEditing} />}
        {formStep === "1" && (
          <ContactAndAddressForm currentForm={data.form} profileId={profileId} isEditing={isEditing} />
        )}
        {formStep === "2" && <PassportForm currentForm={data.form} profileId={profileId} isEditing={isEditing} />}
        {formStep === "3" && <AboutTravelForm currentForm={data.form} profileId={profileId} isEditing={isEditing} />}
        {formStep === "4" && <TravelCompanyForm currentForm={data.form} profileId={profileId} isEditing={isEditing} />}
        {formStep === "5" && <PreviousTravelForm currentForm={data.form} profileId={profileId} isEditing={isEditing} />}
        {formStep === "6" && <USAContactForm currentForm={data.form} profileId={profileId} isEditing={isEditing} />}
        {formStep === "7" && <FamilyForm currentForm={data.form} profileId={profileId} isEditing={isEditing} />}
        {formStep === "8" && <WorkEducationForm currentForm={data.form} profileId={profileId} isEditing={isEditing} />}
        {formStep === "9" && (
          <AdditionalInformationForm currentForm={data.form} profileId={profileId} isEditing={isEditing} />
        )}
        {formStep === "10" && <SecurityForm currentForm={data.form} profileId={profileId} isEditing={isEditing} />}
      </div>
    </>
  );
}
